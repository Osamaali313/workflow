...
Nitro integration for Workflow SDK. The `workflow/nitro` entry point's default
export is a [Nitro module](https://v3.nitro.build/guide/modules) — it has no
callable API. Enable it by adding it to the `modules` array of your Nitro config.
...
Shared build settings can also be placed in
[`workflow.config.ts`](/docs/foundations/configuration). Nitro-specific settings
remain under `workflow` in `nitro.config.ts`.
