...
Use [`workflow.config.ts`](/docs/foundations/configuration) for the World and
...
Configure World-specific options in `workflow.world.ts`. The optional second
argument to `withWorkflow` overrides environment variables and shared config.
...
`true`. Precedence is: explicit `withWorkflow` option > `WORKFLOW_SOURCEMAP` >
`workflow.config.ts` > the environment-aware default (`'inline'` in
development, `false` in production). Development is detected from `next dev` /
`NODE_ENV=development`, so the framework option, environment variable, and
shared config can each force either behavior in either environment.
