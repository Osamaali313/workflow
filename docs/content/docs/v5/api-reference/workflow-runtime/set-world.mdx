...
type: reference
summary: Use setWorld to inject a custom World instance or reset the managed cache.
prerequisites:
...
returns. Pass `undefined` to clear the cache so the next call resolves the
active target override, configured provider, or automatic World again.
...
|-----------|------|-------------|
| `world` | `World \| undefined` | The World instance to use, or `undefined` to resolve the active configuration again on next access |
