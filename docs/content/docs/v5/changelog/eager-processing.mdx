...
The inline execution loop checks wall-clock time before each replay iteration. If the elapsed time exceeds a configurable threshold (default: 120 seconds), the handler re-schedules itself via the queue and returns. Configurable via `runtime.inlineExecutionTimeoutMs` in `workflow.config.ts`.
