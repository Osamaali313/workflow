...
3. The provider selected in `workflow.config.ts`, or `start()` called when you construct the World directly

<Callout type="info">
The Postgres World is not compatible with Vercel deployments. Omit `world` to
select the [Vercel World](/worlds/vercel) automatically on Vercel, or select an
