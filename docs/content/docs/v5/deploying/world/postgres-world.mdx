...
If you construct a Postgres World outside Workflow configuration, call
`start()` yourself during server initialization.
...
3. The World selected in `workflow.config.ts`
...
appropriate World in `workflow.world.ts` based on the deployment environment.
</Callout>

## Limitations

- **Requires long-running process** - Configured Worlds start automatically; Worlds created elsewhere require `start()` during server initialization
