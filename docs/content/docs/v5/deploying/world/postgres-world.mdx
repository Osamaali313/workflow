...
const world = createWorld() satisfies World;
...
Workflow starts the configured World the first time `getWorld()` runs. Call
`getWorld()` during server startup so Postgres begins processing queued work
immediately.
...
Configure the World in `workflow.world.ts`:
...
3. `getWorld()` called during server initialization

<Callout type="info">
Do not configure the Postgres World for Vercel deployments. When `world` and
`WORKFLOW_TARGET_WORLD` are omitted, Workflow automatically uses the
[Vercel World](/docs/deploying/world/vercel-world).
</Callout>

## Limitations

- **Requires long-running process** - Call `getWorld()` during server startup so the worker starts polling for jobs
